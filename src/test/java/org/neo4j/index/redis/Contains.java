...
public class Contains<T> extends TypeSafeMatcher<IndexHits<T>>
{
    ...
    public Contains( Collection<T> expectedItems )
    {
    ...
    public boolean matchesSafely( IndexHits<T> tIndexHits )
    {
        ...
        if ( expectedItems.size() != collection.size() )
        {
            message = "IndexHits with a size of " + expectedItems.size() + ", got one with " + collection.size();
    ...
    {
        return new Contains<T>( Arrays.asList( expectedItems ) );
    }

    @Factory
    public static <T> Contains<T> contains( Collection<T> expectedItems )
    {
